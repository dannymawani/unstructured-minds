...
export function WidgetConfigPanel({ widgets, onChange }: WidgetConfigPanelProps) {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-popover text-popover-foreground rounded-md shadow-lg border border-border z-30 overflow-hidden">
          {/* Header */}
          ...
          <div className="max-h-80 overflow-y-auto">
            {sortedWidgets.map(({ config, definition }) => (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3 hover:bg-secondary/50 cursor-move transition-colors ${
                  draggedId === config.id ? 'opacity-50 bg-secondary' : ''
                }`}
              >
